...
import { 
  ...
  ShieldCheck, Stamp, Bell, Loader2
} from "lucide-react";
  ...
  const topLevelMetrics = useMemo(() => {
    if (loading) {
      return [
        ...
        { label: "Internal Alerts", value: "-", icon: Bell, desc: "Loading...", colorClass: "text-rose-500", bgClass: "bg-rose-500/10 border-rose-500/20" }
      ];
    }
    return [
      ...
      { label: "Internal Alerts", value: notifStats.total.toString(), icon: Bell, desc: `${notifStats.unread} Unread Messages • ${notifStats.activeAlerts} Action Required`, colorClass: "text-rose-500", bgClass: "bg-rose-500/10 border-rose-500/20" }
    ];
    ...
    <div className="mx-auto max-w-7xl space-y-8 pb-14 px-4 sm:px-6 lg:px-8">
        ...
        <div>
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Badge variant="outline" className="text-muted-foreground font-medium px-3 py-1">
              Search, Landscape, and Evidence Intelligence
            </Badge>
          </div>
          ...
          <p className="mt-2 text-sm leading-relaxed text-muted-foreground/80 max-w-3xl">
            Real-time analytical oversight of global intellectual property assets, brand protection, and legal defense systems.
          </p>
        </div>
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {topLevelMetrics.map((metric, idx) => (
          <Card key={idx} className="border-border/60 bg-card/60 glass-card">
